...
if __name__ == '__main__':
    ...
    for i in range(N):
        ...
        except (TypeError, FileNotFoundError, PermissionError) as err: # catch "buffer is too small for requested array" error when reading corrupted files
            if do_logging:
...
#            raise
            try:
                y, trend = flatten(star.x, star.y, **wotan_kwargs)
            except ValueError("all nans in PDCSAP photometry"):
                flag = -4
                raise
...
#            print(f"{_tic}: {len(freqs)} freqs, flag {flag}, mass {star.mass.value}, radius {star.radius.value}, span {np.max(x) - np.min(x)}")

            if flag != 0:
                raise ValueError("negative flag after detrending and frequency grid creation")
